...
def climbingLeaderboard(scores, alice):
    ...
    for alice_score in alice:
        j = 0
        while (j < len(list_of_score_ranks)):
            if(alice_score < list_of_score_ranks[j][0]):
                j+=1
            elif (alice_score == list_of_score_ranks[j][0]):
                alice_rankz.append(list_of_score_ranks[j])
                j +=1
            else:
                alice_rankz.append(1)
                j+=1
